...
#             dim_max_poss_conversion=(np.trunc(self.d_param[dim]["param c"]*100)/100)
    ...
    def adjust_budget(self, newSpendVec, totalReturn, dimension_bound_actual, budgetGoal, newImpVec):
        ...
        if self.use_impression:
            d_median_spend = {dim: ((value['impression_median'] * self.d_param[dim]['cpm'])/1000) for dim, value in self.d_param.items()}
        else:
        ...
        for dim in self.d_param:
            ...
            if(round(return_projection[dim])>=self.d_param[dim]['param c']):
                continue
    ...
    def lift_cal(self, result_df, budget_per_day, df_spend_dis, days, dimension_bound):
        ...
        """
        if self.use_impression:
            result_df.rename({self.metric: 'recommended_impressions_per_day', 'return': 'estimated_return_per_day'}, axis=1, inplace=True)
            ...
            result_df['recommended_budget_per_day']=((result_df['cpm']*result_df['recommended_impressions_per_day'])/1000).round().astype(int)
        else:
